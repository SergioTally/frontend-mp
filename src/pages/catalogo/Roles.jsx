...
const columnas = [
  { campo: "ID_ROLE", label: "ID", visible: true, tipo: "hidden" },
  {
    campo: "NOMBRE",
...
const PTRole = () => {
  ...
  const handleDelete = async (id) => {
    try {
      await api.delete(`/ptRole/${id}`);
      setItems((prev) => prev.filter((item) => item.ID_ROLE !== id));
      showSnackbar("Rol eliminado correctamente");
    } catch {
      setError("No se pudo eliminar el rol.");
      showSnackbar("No se pudo eliminar el rol", "error");
    }
  };
  ...
  const handleDialogConfirm = () => {
    switch (dialogAction) {
      case "delete":
        handleDelete(selectedItem.ID_ROLE);
        break;
      case "edit":
  ...
  const handleAdd = async () => {
    try {
      if (editing) {
        await api.put(`/ptRole/${formData.ID_ROLE}`, formData);
        await fetchItems();
        showSnackbar("Rol actualizado correctamente");
      } else {
        const { data } = await api.post("/ptRole", formData);
        setItems((prev) => [...prev, data]);
        showSnackbar("Rol agregado correctamente");
      }
      setFormOpen(false);
      setFormData({});
      setEditing(false);
  ...
  return (
    ...
    >
      ...
      <ConfirmDialog
        ...
        message={`¿Deseas continuar con la acción de ${dialogAction} para el rol "${selectedItem?.NOMBRE}"?`}
